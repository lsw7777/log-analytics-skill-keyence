...
function Get-SuspiciousIpSlidingWindowRows {
    ...
    foreach ($group in @($Rows | Group-Object -Property IP)) {
        ...
        if ($maxWindowCount -ge $effectiveThreshold) {
            ...
            # 聚合该IP关联的用户/应用信息（去重）
            $identities = @($events | ForEach-Object { $_.Identity } | Where-Object { $_ } | Sort-Object -Unique) -join ', '
            $userDisplayNames = @($events | ForEach-Object { $_.UserDisplayName } | Where-Object { $_ } | Sort-Object -Unique) -join ', '
            $userPrincipalNames = @($events | ForEach-Object { $_.UserPrincipalName } | Where-Object { $_ } | Sort-Object -Unique) -join ', '
            $locations = @($events | ForEach-Object { $_.Location } | Where-Object { $_ } | Sort-Object -Unique) -join ', '
            $appDisplayNames = @($events | ForEach-Object { $_.AppDisplayName } | Where-Object { $_ } | Sort-Object -Unique) -join ', '
            $clientAppsUsed = @($events | ForEach-Object { $_.ClientAppUsed } | Where-Object { $_ } | Sort-Object -Unique) -join ', '
    ...
    foreach ($row in $dataset.Rows) {
        ...
        if ($table -eq 'SigninLogs' -and $isUsablePublicIp -and -not $isTrustedIp) {
            ...
            $suspiciousIpRecords.Add([PSCustomObject]@{
                ...
                FirstTime = Get-LocalTimeText -Value $firstTime
                LastTime = Get-LocalTimeText -Value $lastTime
                Identity = Get-AnyFieldValue -Row $row -Names @('Identity') -Default ''
                UserDisplayName = Get-AnyFieldValue -Row $row -Names @('UserDisplayName') -Default ''
                UserPrincipalName = Get-AnyFieldValue -Row $row -Names @('UserPrincipalName') -Default ''
                Location = Get-AnyFieldValue -Row $row -Names @('Location') -Default ''
                AppDisplayName = Get-AnyFieldValue -Row $row -Names @('AppDisplayName') -Default ''
                ClientAppUsed = Get-AnyFieldValue -Row $row -Names @('ClientAppUsed') -Default ''
            }) | Out-Null
        }
...
| extend __isPublicIp = isnotempty(__ip) and not(ipv4_is_private(__ip))
| extend __isExcludedIp = __isPublicIp and ipv4_is_in_any_range(__ip, dynamic([
    "116.214.36.32/32",
    "101.133.145.90/32",
    "139.224.232.176/32",
    "116.214.36.37/32",
...
| where __isPublicIp and not(__isExcludedIp)
| project TimeGenerated, IPAddress=__ip, Identity, UserDisplayName, UserPrincipalName, Location, AppDisplayName, ClientAppUsed
| order by TimeGenerated desc
...
$failedSigninHtml = (New-CodeBlockHtml -Text $failedSigninKql) + (New-TableHtml -Rows ($failedSigninFiltered | Select-Object -First 10000) -Columns @('风险等级', '次数', '最后时间', 'IP', '主体/应用摘要', '说明') -CellBuilder {
    ...
    $riskReason = Get-RiskReason -Category 'FailedSignins' -Row $r
...
$suspiciousIpHtml = (New-CodeBlockHtml -Text $suspiciousIpKql) + $suspiciousIpTableStyle + $suspiciousIpTableWrapperOpen + (New-TableHtml -Rows $suspiciousIpRows -Columns @('风险等级', 'IP', '次数', '首次访问时间', '最近访问时间', 'UserDisplayName', 'UserPrincipalName', 'AppDisplayName', 'ClientAppUsed') -CellBuilder {
    param($r) 
    $riskLevel = Get-RiskLevel -Category 'SuspiciousIP' -Row $r
    $riskReason = Get-RiskReason -Category 'SuspiciousIP' -Row $r
    $riskBadge = Get-RiskLevelBadge -Level $riskLevel -Reason $riskReason
    ...
    $userDisplayName = if ($r.PSObject.Properties.Name -contains 'UserDisplayName') { $r.UserDisplayName } else { '' }
    $userPrincipalName = if ($r.PSObject.Properties.Name -contains 'UserPrincipalName') { $r.UserPrincipalName } else { '' }
    $appDisplayName = if ($r.PSObject.Properties.Name -contains 'AppDisplayName') { $r.AppDisplayName } else { '' }
    $clientAppUsed = if ($r.PSObject.Properties.Name -contains 'ClientAppUsed') { $r.ClientAppUsed } else { '' }
    $cellScrollOpen = '<span class="sip-cell-scroll">'
    $cellScrollClose = '</span>'
    @($riskBadge, $r.IP, $r.Count, $firstAccess, $lastAccess, ($cellScrollOpen + (Escape-Html $userDisplayName) + $cellScrollClose), ($cellScrollOpen + (Escape-Html $userPrincipalName) + $cellScrollClose), ($cellScrollOpen + (Escape-Html $appDisplayName) + $cellScrollClose), $clientAppUsed)
} -RawHtmlColumns @('风险等级', 'UserDisplayName', 'UserPrincipalName', 'AppDisplayName')) + $suspiciousIpTableWrapperClose
...
$licenseTableRows = @(
    foreach ($license in $licenseUsage) {
        ...
        $riskLevel = Get-LicenseRiskLevel -Remaining $remaining
        $riskReason = Get-LicenseRiskReason -Remaining $remaining
        $riskBadge = Get-RiskLevelBadge -Level $riskLevel -Reason $riskReason
        [PSCustomObject]@{
...
$licenseHtml = (New-CodeBlockHtml -Text $licenseLogic) + (New-TableHtml -Rows $licenseTableRows -Columns @('风险等级', 'SkuPartNumber', '产品名称', '总数', '已分配', '剩余', '状态') -CellBuilder {
    param($r) @($r.RiskBadge, $r.SkuPartNumber, $r.ProductName, $r.Total, $r.Used, $r.Remaining, $r.Status)
} -RawHtmlColumns @('风险等级', '状态'))

# 计算 License 栏整体风险等级（取所有 License 中最高风险）
$licenseOverallRisk = 'none'
foreach ($license in $licenseUsage) {
    $remaining = if ($null -ne $license.Remaining -and $license.Remaining -ne 'N/A') { [int]$license.Remaining } else { 0 }
    $riskLevel = Get-LicenseRiskLevel -Remaining $remaining
    if ($riskLevel -eq 'medium') {
        $licenseOverallRisk = 'medium'
        break
    } elseif ($riskLevel -eq 'low' -and $licenseOverallRisk -eq 'none') {
        $licenseOverallRisk = 'low'
    }
}
...
$dcrLogErrorHtml = (New-CodeBlockHtml -Text $dcrLogErrorsKql) + (New-TableHtml -Rows ($dcrLogErrorRows | Select-Object -First 10000) -Columns @('风险等级', '次数', '时间', '输入流ID', '操作名称', '消息') -CellBuilder {
    ...
    $riskReason = Get-RiskReason -Category 'DcrLogErrors' -Row $r
    $riskBadge = Get-RiskLevelBadge -Level $riskLevel -Reason $riskReason
...
$permissionHtml = (New-CodeBlockHtml -Text $permissionKql) + (New-TableHtml -Rows ($identityPermissionChanges | Sort-Object -Property ActivityDateTime -Descending) -Columns @('风险等级', '活动时间', '操作者', '操作', '结果/说明') -CellBuilder {
    ...
    $riskReason = Get-RiskReason -Category 'IdentityPermission' -Row $r
    $riskBadge = Get-RiskLevelBadge -Level $riskLevel -Reason $riskReason
    $permValue = $r.Detail
...
$intuneHtml = (New-CodeBlockHtml -Text $intuneAuditKql) + (New-TableHtml -Rows ($intuneGrouped | Select-Object -First 10000) -Columns @('风险等级', '次数', '最后时间', '操作者', '操作', '目标', '结果/说明') -CellBuilder {
    ...
    $riskReason = Get-RiskReason -Category 'IntuneAudit' -Row $r
    $riskBadge = Get-RiskLevelBadge -Level $riskLevel -Reason $riskReason
...
.layout { display: grid; grid-template-columns: 200px minmax(0, 1fr); gap: 16px; max-width: 1850px; margin: 0; padding: 10px 10px 10px 0; }
.wrap { min-width: 0; }
.side-nav { position: sticky; top: 10px; align-self: start; max-height: calc(100vh - 20px); overflow: auto; background: var(--panel); border: 1px solid var(--line); border-radius: 8px; padding: 8px; margin-left: 10px; }
