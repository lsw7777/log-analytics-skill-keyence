...
Assert-Equal $config.Tables.Count 8 'Default schedule table count mismatch.'
Assert-Equal $config.Tables[0] 'AADManagedIdentitySignInLogs' 'Default schedule first table mismatch.'
Assert-Equal $config.Tables[7] 'SigninLogs' 'Default schedule last table mismatch.'

$customConfig = New-LogAnalyzerScheduleConfig -RunAt '03:30' -Tables @('AuditLogs', 'SigninLogs')
