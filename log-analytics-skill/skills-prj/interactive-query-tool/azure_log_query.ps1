...
function Invoke-LogQuery {
    ...
    if ($response.Results) {
        ...
        # $response.Results
        # $totalColumns = $response.Results[0].PSObject.Properties.Name.Count
        # $totalRows = $response.Results.Count

        # Write-Host "`n--- Results: $totalRows rows, $totalColumns columns ---" -ForegroundColor Green

        # Output as table
        # $response.Results | Format-Table -AutoSize | Out-String | Write-Host

        # Write-Host "`nQuery completed successfully! ($totalRows rows total)" -ForegroundColor Green
    }
    else {
