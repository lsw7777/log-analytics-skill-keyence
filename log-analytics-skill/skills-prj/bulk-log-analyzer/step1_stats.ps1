...
foreach ($row in $data) {
    if ($row.TimeGenerated) {
        try {
            $localDt = ([DateTime]::Parse($row.TimeGenerated)).ToLocalTime()
...
$offHoursByUserSorted = $offHoursByUser.GetEnumerator() | Sort-Object Value -Descending | Select-Object -First 15
...
$highPrivByUser = @{}
foreach ($row in $data) {
    if ($highPrivOps -contains $row.Operation) {
        $u = if ($row.UserUPN) { $row.UserUPN } elseif ($row.UserId) { $row.UserId } else { 'Unknown' }
        $key = "$u | $($row.Operation)"
...
$stats = @{
    ...
    offHoursUsers = ($offHoursByUserSorted | ForEach-Object { @{ name = $_.Name; value = $_.Value } })
    ...
    riskCount = $(
        ...
        if ($offHoursByUser.Count -gt 0) { $c++ }
