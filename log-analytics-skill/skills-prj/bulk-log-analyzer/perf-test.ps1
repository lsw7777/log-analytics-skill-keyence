...
foreach ($table in $Tables) {
    ...
    try {
        ...
        $shortName = $table -replace 'DCR_CL$', '' -replace 'AuditGeneral$', 'General' -replace 'SharePointAudit$', 'SPAudit' -replace 'MessageTraceData$', 'MsgTrace' -replace 'AzureADUsers$', 'AADUsers' -replace 'MailboxStatistics$', 'Mailbox' -replace 'WQCLog$', 'WQC' -replace 'AssignedLicenses$', 'Licenses'
...
| WQCLogDCR_CL | WQC日志 | Result (Success/Failure) |
